...
const getMockDetectedRisks = (): RiskDetectionResponse => {
  const db = getMockDb();
...
const getMockProjectsSummary = (): ProjectSummaryResponse => {
  const db = getMockDb();
...
export const detectRisks = async (): Promise<RiskDetectionResponse> => {
  if (getApiMode() === 'mock') {
    ...
    return getMockDetectedRisks();
  }

  const response = await apiClient.post<any>(`/api/v1/ai/detect-risks`);
...
export const getProjectsSummary = async (): Promise<ProjectSummaryResponse> => {
  if (getApiMode() === 'mock') {
    ...
    return getMockProjectsSummary();
  }

  const response = await apiClient.get<ProjectSummaryResponse>(`/api/v1/ai/projects/summary`);
  return response.data;
};

export const getProjectSummary = async (projectId: string | number): Promise<{ summary: string }> => {
  if (getApiMode() === 'mock') {
    await new Promise((resolve) => setTimeout(resolve, 800));
    return { summary: `Mock project summary for project ${projectId}` };
  }
  const response = await apiClient.get<{ summary: string }>(`/api/v1/ai/project-summary/${projectId}`);
  return response.data;
};
